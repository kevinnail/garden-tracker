...
| 2026-07-21 | "Today" date label (planner toolbar banner + Today screen) froze on launch-day date across midnight in a long-running session | `useTodayTick()` now returns the local day captured at rollover, anchored to local **noon** (immune to Hermes' ~1hr timezone-offset cache wobble near midnight); labels format that value via `formatDateLabel()` (raw getters, no `toLocaleDateString`) instead of a render-time `new Date()` (`useTodayTick.ts`, `dateUtils.ts`, `PlannerToolbar.tsx`, `today.tsx`) | ✅ dev build, manual clock-jump — both pages update, reproducible (real-midnight not yet observed) | ⏳ deferred | — |
| 2026-07-21 | FOLLOW-UP (not yet fixed): grid today-cursor + column highlight use render-time `new Date()` via `todayWeekIndex()` (`TodayCursor.tsx`, `PlannerGrid.tsx`, `GridBody.tsx`) — same root cause, can land wrong-column across midnight | Thread `useTodayTick()`'s noon-anchored day through instead of `todayWeekIndex(new Date())` | ⬜ TODO | ⏳ deferred | — |
